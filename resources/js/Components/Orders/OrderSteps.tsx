...
export default function OrderSteps({ status, createdAt, dueDate, history }: OrderStepsProps) {
    ...
    const steps = [
        { id: 'new',         label: 'NEW',         icon: Check,    date: getStatusDate('new') || new Date(createdAt).toLocaleDateString(undefined, { month: 'short', day: 'numeric' }) },
        { id: 'in_progress', label: 'IN PROGRESS', icon: Activity,  date: getStatusDate('in_progress') },
        { id: 'finished',    label: 'FINISHED',    icon: Clock,     date: getStatusDate('finished') },
        { id: 'shipped',     label: 'SHIPPED',     icon: Truck,     date: getStatusDate('shipped') },
        { id: 'delivered',   label: 'DELIVERED',   icon: Flag,      date: getStatusDate('delivered') || `Est. ${new Date(dueDate).toLocaleDateString(undefined, { month: 'short', day: 'numeric' })}` },
    ];
