            ...
            <div className="glass-card rounded-2xl p-6 animate-fade-in animate-delay-400">
                ...
                <form onSubmit={handleAddNote} className="mb-6">
                    <div className="relative group">
                        <textarea
                            ...
                            className="w-full bg-gray-50 dark:bg-slate-900/40 border border-gray-100 dark:border-slate-700/50 rounded-xl p-4 text-sm focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500 transition-all min-h-[100px] resize-none"
                        />
                        <button
                            type="submit"
                ...
                <div className="space-y-4 max-h-[300px] overflow-y-auto pr-2 custom-scrollbar">
                    {notes && notes.length > 0 ? (
                        notes.map((note) => (
                            <div key={note.id} className="relative group p-4 bg-white/50 dark:bg-slate-800/40 rounded-xl border border-gray-100 dark:border-slate-700/50 hover:border-amber-200 dark:hover:border-amber-900/30 transition-all">
                                <div className="flex justify-between items-start mb-2">
                                    <div className="flex items-center gap-2">
                                        ...
                                        <span className="text-xs font-bold text-gray-700 dark:text-gray-200">{note.user.name}</span>
                                    </div>
                                    <div className="flex items-center gap-2">
                                ...
                                <p className="text-sm text-gray-600 dark:text-gray-300 whitespace-pre-wrap leading-relaxed">{note.content}</p>
                            </div>
                        ))
                    ) : (
                        <div className="text-center py-6 border-2 border-dashed border-gray-100 dark:border-slate-800 rounded-2xl">
                            <p className="text-xs text-gray-400 italic">{t('No internal notes yet.')}</p>
                        </div>
                    )}
                </div>
            </div>
