            ...
            <div className="max-w-4xl mx-auto flex flex-col gap-6 pb-12">
                ...
                <div className="flex items-center justify-between no-print">
                    <div className="flex items-center gap-4">
                        <Link href={route('lab.invoices.index')} className="p-2 rounded-lg hover:bg-[var(--surface)] transition-colors text-slate-400">
                            <ArrowLeft size={18} />
                        </Link>
                        <div>
                            <h2 className="text-[20px] font-semibold tracking-tight text-white">{invoice.invoice_number}</h2>
                            <p className="text-[12.5px] mt-0.5 text-slate-400">{t('Details and status management.')}</p>
                        </div>
                    </div>
                ...
                <div className="card invoice-card overflow-hidden flex flex-col" style={{ border: '1px solid #312e81', background: 'transparent' }}>
                    ...
                    <div className="p-8 border-b border-[#312e81] flex justify-between items-start">
                        <div className="space-y-4">
                            ...
                            <div className="space-y-1">
                                ...
                                <p className="text-[12px] text-slate-400">Laboratory Portal</p>
                            </div>
                        </div>
                        ...
                        <div className="text-right flex flex-col items-end gap-3">
                            ...
                            <div className="space-y-1 mt-2">
                                <p className="text-[11px] font-black uppercase tracking-widest text-slate-500">{t('Invoice Number')}</p>
                    ...
                    <div className="p-8 grid grid-cols-2 gap-8">
                        ...
                        <div className="flex justify-end gap-12 text-right">
                            <div className="space-y-4 text-right">
                                <div>
                                    <p className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500 mb-1">{t('Date Issued')}</p>
                                    <p className="text-[13.5px] font-semibold text-white">{fmtDate(invoice.created_at)}</p>
                                </div>
                                <div>
                                    <p className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500 mb-1">{t('Due Date')}</p>
                        ...
                        <table className="w-full text-left">
                            <thead>
                                <tr className="border-b border-[#312e81]">
                                    <th className="py-4 text-[11px] font-black uppercase tracking-widest text-slate-500">{t('Order Details')}</th>
                                    <th className="py-4 text-[11px] font-black uppercase tracking-widest text-slate-500">{t('Service')}</th>
                                    <th className="py-4 text-[11px] font-black uppercase tracking-widest text-slate-500 text-right">{t('Amount')}</th>
                                </tr>
                            </thead>
                                ...
                                {invoice.orders.map((order) => (
                                    <tr key={order.id}>
                                        <td className="py-4">
                                            ...
                                            <p className="text-[11px] text-slate-500 mt-0.5">{fmtDate(order.created_at)}</p>
                                        </td>
                                        <td className="py-4 text-[13px] text-slate-400">
                                            {order.service.name}
                                        </td>
                    ...
                    <div className="bg-[rgba(15,23,42,0.1)] p-8 flex justify-between items-start border-t border-[#312e81]">
                        <div className="max-w-xs">
                            <p className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500 mb-2">{t('Notes')}</p>
                        ...
                        <div className="text-right space-y-3">
                            <div className="flex justify-end gap-12 items-center">
                                <span className="text-[12px] font-bold text-slate-500 uppercase tracking-widest">{t('Subtotal')}</span>
