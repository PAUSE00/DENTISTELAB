...
const INPUT_BASE = "w-full px-4 py-2.5 rounded-lg text-[13px] border border-[#312e81] focus:outline-none focus:ring-1 focus:ring-indigo-500 focus:border-accent transition-colors placeholder:";
...
export default function Create({ clinics }: Props) {
 ...
 <button onClick={() => window.history.back()} className="p-2 rounded-lg hover:bg-[var(--surface)] transition-colors ">
 <ArrowLeft size={18} />
 </button>
 <div>
 <h2 className="text-[20px] font-semibold tracking-tight text-white">{t('Generate New Invoice')}</h2>
 <p className="text-[12.5px] mt-0.5 ">{t('Select clinic and uninvoiced orders.')}</p>
 </div>
 </div>
 </div>
 ...
 <span className="text-[12px] font-bold uppercase tracking-wider">{t('Total Amount')}</span>
 ...
 <h3 className="text-[13px] font-bold uppercase tracking-widest">{t('Select Orders')}</h3>
 {orders.length > 0 && (
 ...
 <p className=" text-[14px]">{t('Please select a clinic to see available orders.')}</p>
 </div>
 ) : loading ? (
 ...
 <p className=" text-[14px]">{t('No uninvoiced orders found for this clinic.')}</p>
 </div>
 ) : (
