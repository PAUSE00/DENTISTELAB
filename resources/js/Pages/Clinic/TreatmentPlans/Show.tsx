            ...
            <div className="p-6 max-w-5xl mx-auto space-y-6">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    ...
                    <div className="flex items-center gap-3">
                        <button className="flex items-center gap-2 px-4 py-2 bg-slate-800 hover:bg-slate-700 text-white font-bold rounded-xl transition-all">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div className="lg:col-span-2 space-y-6">
                        <div className="bg-slate-900 border border-slate-800 p-6 rounded-3xl shadow-sm relative overflow-hidden">
                            ...
                            <div className="space-y-0 relative">
                                {/* Connector Line */}
                                <div className="absolute left-[19px] top-4 bottom-12 w-0.5 bg-slate-800" />
                                ...
                                {plan.steps.map((step: any, idx: number) => {
                                    ...
                                    return (
                                        <div key={step.id} className={`relative pl-12 pb-10 last:pb-0 group transition-all`}>
                                            {/* Node */}
                                            <div 
                                                className={`absolute left-0 w-10 h-10 rounded-full border-4 border-slate-900 ring-2 ring-slate-800 z-10 flex items-center justify-center transition-all bg-slate-950`}
                                                style={{ color: st.color }}
                                            >
                                                <StepIcon size={18} className="" />
                                            </div>

                                            <div className={`p-5 rounded-2xl border transition-all ${isExpanded ? 'bg-slate-950 border-slate-700/50 shadow-xl' : 'bg-slate-950/20 border-slate-800/30 hover:bg-slate-950/40 hover:border-slate-800'}`}>
                                                <div className="flex items-start justify-between">
                                                    <div className="cursor-pointer flex-1" onClick={() => setExpandedStep(isExpanded ? null : idx)}>
                                                        <h3 className={`font-black text-sm transition-colors ${step.status === 'done' ? 'text-slate-500 line-through' : 'text-white'}`}>
                                                            {step.title}
                                                        </h3>
                                                        <div className="flex items-center gap-3 mt-1 text-[10px] uppercase font-black tracking-widest text-slate-500">
                                                ...
                                                {isExpanded && (
                                                    <div className="mt-4 pt-4 border-t border-slate-800 space-y-4 animate-in slide-in-from-top-2 duration-300">
                                                        <div className="text-xs text-slate-400 leading-relaxed italic border-l border-slate-800 pl-3">
                                                        ...
                                                        <div className="flex items-center gap-2 pt-2">
                                                            <select 
                                                                ...
                                                                className="bg-slate-900 border-none text-[10px] font-black uppercase tracking-widest text-slate-400 rounded-lg px-2 py-1 outline-none"
                                                            >
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="bg-slate-900 border border-slate-800 p-6 rounded-3xl space-y-6 shadow-sm">
                            <h2 className="text-sm font-black uppercase tracking-widest text-white border-b border-slate-800 pb-3">{t('Treatment Velocity')}</h2>
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="grid grid-cols-1 gap-3 pt-2">
                                    <div className="bg-slate-950 p-4 rounded-3xl border border-slate-800">
                                        <div className="text-[10px] font-bold text-slate-500 uppercase mb-1 tracking-widest">{t('Estimated Cost')}</div>
                                        <div className="text-2xl font-black text-white">{formatCur(plan.estimated_cost)}</div>
                                    </div>
                                    <div className="bg-slate-950 p-4 rounded-3xl border border-slate-800">
                                        <div className="text-[10px] font-bold text-slate-500 uppercase mb-1 tracking-widest">{t('Case Owner')}</div>
                                        <div className="text-sm font-black text-white flex items-center gap-2">
                                            <div className="w-5 h-5 rounded-full bg-slate-800 flex items-center justify-center text-[10px]">{plan.creator.name.charAt(0)}</div>
                                            {plan.creator.name}
                                        </div>
                                    </div>
                                </div>
                        ...
                        <div className="bg-slate-900 border border-slate-800 p-6 rounded-3xl space-y-4">
                            <div className="flex items-center gap-2">
                                ...
                                <h3 className="text-[10px] font-black text-slate-500 uppercase tracking-widest">{t('Case Narrative')}</h3>
                            </div>
