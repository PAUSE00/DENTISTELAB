...
export default function Create({ auth, patients, labs, templates, duplicate }: Props) {
 ...
 // Step 2
 teeth: (duplicate?.teeth ? (Array.isArray(duplicate.teeth) ? duplicate.teeth.reduce((acc: Record<number, string>, t: number) => ({...acc, [t]: 'Crown'}), {}) : duplicate.teeth) : {}) as Record<number, string>,
 ...
 if (template.teeth_data) {
 const mappedTeeth = Array.isArray(template.teeth_data) ? template.teeth_data.reduce((acc: any, t: number) => ({...acc, [t]: 'Crown'}), {}) : template.teeth_data;
 setData('teeth', mappedTeeth);
 }
 ...
 const assignTreatment = (toothNumber: number, treatment: string | null) => {
 const copy: Record<number, string> = { ...data.teeth };
 if (treatment) {
 copy[toothNumber] = treatment;
 } else {
 ...
 <Odontogram
 selectedTeeth={Object.keys(data.teeth).map(Number)}
 toothTreatments={data.teeth as Record<number, string>}
 onAssignTreatment={assignTreatment}
 />
 </div>
 ...
 {t('Selected teeth')}: <span className="font-bold" style={{ color: 'var(--teal)' }}>{Object.keys(data.teeth).length > 0 ? Object.entries(data.teeth).map(([k, v]) => `${k} (${v})`).join(', ') : 'None'}</span>
 </p>
 </div>
 ...
 <button type="button" onClick={(e) => { e.stopPropagation(); removeFile(index); }} className="p-2 transition-colors hover:text-red-500" style={{ color: 'var(--txt-3)' }}>
 <X className="w-4 h-4" />
 </button>
 </div>
 ))}
 ...
 >
 <span className="font-bold">{t('Next Step')}</span>
 ...
 <span className="font-bold">{t('Submit Order')}</span>
 </button>
 )}
 </div>
 </form>
