...
export default function Edit({ auth, order, patients, labs }: Props) {
 ...
 teeth: (order.teeth ? (Array.isArray(order.teeth) ? order.teeth.reduce((acc: Record<number, string>, t: number) => ({...acc, [t]: 'Crown'}), {}) : order.teeth) : {}) as Record<number, string>,
 shade: order.shade ?? '',
 ...
 <span className="font-bold">{t('Update Order')}</span>
 </button>
 )}
 </div>
 </form>
