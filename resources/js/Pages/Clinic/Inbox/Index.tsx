...
export default function Inbox({ auth, conversations }: Props) {
 ...
 const lName = c.lab?.name?.toLowerCase() || '';
 return pName.includes(q) || lName.includes(q) || `ORD-${c.id}`.includes(q);
 });
 ...
 <div className="w-1.5 h-6 rounded-full" style={{ background: 'var(--teal)' }} />
 ...
 <span className="text-[10px] font-bold px-2 py-0.5 rounded-full" style={{ background: 'var(--teal-10)', color: 'var(--teal)' }}>
 {filtered.length}
 </span>
 </div>
 ...
 const initials = getInitials(conv.lab?.name || '');
 ...
 >
 {isActive && (
 <div className="absolute left-0 top-1/2 -translate-y-1/2 w-1 h-8 rounded-r-md shadow-[0_0_10px_var(--teal)]" style={{ background: 'var(--teal)' }} />
 )}
 
 {/* Avatar */}
 <div className="relative shrink-0">
 <div className="w-11 h-11 rounded-full flex items-center justify-center font-bold text-sm text-white"
 style={{ background: isActive ? 'var(--teal)' : 'var(--border-strong)' }}>
 {initials}
 </div>
 <div className="absolute bottom-0 right-0 w-3 h-3 border-2 rounded-full" style={{ background: '#10b981', borderColor: 'var(--surface)' }} />
 </div>
 ...
 <div className="relative shrink-0">
 <div className="w-10 h-10 text-white rounded-full flex items-center justify-center font-bold shadow-lg" style={{ background: 'var(--teal)' }}>
 {getInitials(activeConv.lab?.name)}
 </div>
 <div className="absolute bottom-0 right-0 w-2.5 h-2.5 border-2 rounded-full" style={{ background: '#10b981', borderColor: 'var(--surface)' }} />
 </div>
 <div className="flex flex-col">
 <span className="text-[15px] font-bold" style={{ color: 'var(--txt-1)' }}>{activeConv.lab?.name}</span>
 ...
 {getInitials(msg.user_name || activeConv.lab?.name)}
 </div>
 )}
 ...
 style={{ 
 background: isOwn ? 'var(--teal)' : 'var(--surface)',
 color: isOwn ? '#fff' : 'var(--txt-1)',
 borderColor: isOwn ? 'var(--teal)' : 'var(--border)',
 ...
 <CheckCheck size={14} style={{ color: 'var(--teal)' }} />
 </div>
 ) : (
 ...
 <form onSubmit={sendMessage} className="flex items-center border rounded-full p-1.5 shadow-lg group focus-within:border-[var(--teal)] transition-colors"
 style={{ background: 'var(--surface)', borderColor: 'var(--border)' }}>
 ...
 <button type="submit" disabled={!newMessage.trim()} className="w-10 h-10 rounded-full text-white flex items-center justify-center transition-colors disabled:opacity-50 shadow-[0_0_15px_var(--teal)]"
 ...
 <h2 className="text-xl font-bold mb-2 z-10" style={{ color: 'var(--txt-2)' }}>Welcome to Inbox</h2>
