...
export default function Index({ auth, invoices, transactions, stats }: Props) {
 ...
 <FileText size={20} className="text-accent" /> {t('Recent Invoices')}
 </h3>
 </div>

 <div className="flex flex-col gap-3">
 {invoices.length > 0 ? invoices.map((invoice) => (
 <div key={invoice.id} className="card p-4 flex items-center justify-between group hover:border-accent/50 transition-all" style={{ background: 'var(--bg-raised)' }}>
 ...
 <Link href={route('clinic.billing.invoice.show', invoice.id)} className="p-2 rounded-lg bg-accent/10 text-accent hover:bg-accent/20 transition-all">
 ...
 <h3 className="text-lg font-bold flex items-center gap-2" style={{ color: 'var(--txt-1)' }}>
 <CreditCard size={20} className="text-accent" /> {t('Cash Record')}
 </h3>
