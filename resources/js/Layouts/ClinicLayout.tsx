...
import {
  ...
  TrendingUp, Compass, DollarSign, Bookmark, MessageSquare, Archive, Stethoscope, LifeBuoy
} from 'lucide-react';
...
export default function ClinicLayout({ children, header, fullBleed }: Props) {
  ...
  const mgmtLinks = [
    { path: 'clinic.tickets.index', pat: 'clinic.tickets.*', label: 'Support', Icon: LifeBuoy },
    ...(user.role === 'dentist' ? [
        ...
        <div className={`flex items-center h-14 border-b ${collapsed ? 'justify-center px-3' : 'px-4 gap-3'}`}
          ...
          <div className="w-8 h-8 flex items-center justify-center shrink-0">
            {user.clinic?.logo_path
              ? <img src={`/storage/${user.clinic.logo_path}`} className="w-full h-full object-cover rounded-md" alt="" />
