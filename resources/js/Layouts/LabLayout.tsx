...
export default function LabLayout({ children, header }: Props) {
    ...
    const getPageTitle = () => {
        ...
        if (isActive('lab.team.*')) return t('Team Management');
            ...
            >
                ...
                <div className={`relative flex flex-col items-center gap-3 border-b border-gray-100 dark:border-slate-800 overflow-hidden group/brand shrink-0 transition-all ${isSidebarCollapsed ? 'p-5' : 'p-6'}`}>
                    ...
                    <div className={`relative rounded-2xl bg-gradient-to-br from-primary-500 to-dental-500 flex items-center justify-center text-white font-bold shadow-xl shadow-primary-500/30 group-hover/brand:scale-105 group-hover/brand:rotate-3 transition-all duration-500 z-10 ${isSidebarCollapsed ? 'w-11 h-11 text-xl' : 'w-14 h-14 text-2xl'}`}>
                        {user.lab?.logo_path ? (
                            <img src={`/storage/${user.lab.logo_path}`} className="w-full h-full object-cover rounded-2xl" alt={t('Lab Logo')} />
                        ) : (
                    ...
                    {!isSidebarCollapsed && (
                        <div className="relative z-10 text-center animate-fade-in w-full">
                            <h1 className="font-bold text-lg tracking-tight text-gray-900 dark:text-white truncate" title={user.lab?.name || "DentalLabPro"}>
                                {user.lab?.name || <span>DentalLab<span className="text-primary-500">Pro</span></span>}
                            </h1>
                            <p className="text-[10px] uppercase tracking-widest text-gray-400 dark:text-gray-500 font-semibold mt-0.5 truncate">{t('Laboratory')}</p>
                        </div>
                    )}
                ...
                <nav className={`flex-1 overflow-y-auto overflow-x-hidden no-scrollbar pb-6 ${isSidebarCollapsed ? 'p-3' : 'p-4'}`}>
                    ...
                    {/* Management Section - Restricted to Lab Owner */}
                    {user.role === 'lab_owner' && (
                        <>
                            {!isSidebarCollapsed ? (
                    ...
                    {user.role === 'lab_owner' && (
                        <div className={`mt-6 pt-4 border-t border-gray-100 dark:border-slate-700/50 flex flex-col ${isSidebarCollapsed ? '' : 'mx-2'}`}>
                            <Link
                                href={route('lab.settings.index')}
                                className={`sidebar-link ${route().current('lab.settings.*') ? 'active' : ''} group ${isSidebarCollapsed ? 'justify-center px-0' : ''}`}
                ...
                <header className="sticky top-0 z-40 glass-effect px-6 py-3 h-16 flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div className="hidden md:flex items-center gap-3 pl-4 border-l border-gray-200 dark:border-slate-700">
                            ...
                            <div className="w-8 h-8 rounded-full bg-gradient-to-br from-primary-400 to-primary-500 flex items-center justify-center text-white text-xs font-bold ring-2 ring-white dark:ring-slate-800 shadow-md">
                                {getInitials(user.name)}
                            </div>
                        </div>
                    </div>
